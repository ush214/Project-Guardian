...
async function listWreckDocs() {
    ...
    for (const colPath of exports.READ_COLLECTIONS) {
        const colRef = admin_1.db.collection(colPath);
        const docRefs = await colRef.listDocuments();
        if (!docRefs || docRefs.length === 0)
            continue;
        const snapshots = await admin_1.db.getAll(...docRefs);
...
exports.onMonitoringEventWrite = functions.firestore
    ...
    const wreckRef = admin_1.db.doc(`${collectionId}/${docId}`);
    await admin_1.db.runTransaction(async (tx) => {
        ...
        if (already) {
            ...
            tx.update(wreckRef, {
                alertsUpdatedAt: admin_1.admin.firestore.FieldValue.serverTimestamp(),
