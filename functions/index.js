...
function extractImageUrlsFromText(text) {
  ...
  for (const u of [...md, ...html, ...plain]) {
    if (looksLikeImageUrl(u)) out.push(u);
  }
  return unique(out);
}
  ...
  for (const tf of textFields) {
    ...
    if (typeof val === "string") {
      ...
      for (const v of Object.values(val)) {
        if (typeof v === "string") urls.push(...extractImageUrlsFromText(v));
      }
    }
  }
...
async function cacheImagesForDoc({ appId, docPath, docId }) {
  ...
  if (!snap.exists) return { created: 0, skipped: 0, candidates: 0 };
  ...
  const toCache = uniqueBy(
    candidates.filter(m => m?.url).map(m => ({ ...m, url: String(m.url) })),
    (m) => m.url
...
export const cacheCollectionReferenceMedia = onCall(
  ...
  async (req) => {
    ...
    for (const docSnap of snap.docs) {
      ...
      if (candidates.length) {
        docsWithCandidates++;
        if (samplesWith.length < 10) samplesWith.push({ docId, candidates: candidates.slice(0, 5) });
      } else {
        if (samplesNone.length < 10) samplesNone.push({ docId });
      }
...
export const cacheReferenceMedia = onCall(
  { region: "us-central1", cors: true, memory: "512MiB", timeoutSeconds: 180 },
  async (req) => {
    const { appId, docPath, docId } = req.data || {};
    const uid = req.auth?.uid || null;

    if (!uid) throw new HttpsError("unauthenticated", "Sign-in required.");
    if (!appId || !docPath || !docId) throw new HttpsError("invalid-argument", "Missing appId, docPath, or docId.");
...
// Import and export monitoring functions
