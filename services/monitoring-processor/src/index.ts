...
function resolveCoordinates(wreck: any): { lat: number, lng: number } | null {
  const toNum = (v: any) => {
  ...
  function extract(obj: any): { lat: number, lng: number } | null {
    if (!obj) return null;
...
function estimatePGA(mmi: number): number {
...
// Process earthquakes
async function processEarthquakes(wrecks: any[]) {
  try {
    ...
    for (const eq of earthquakes) {
      ...
      if (!eqCoords || eqCoords.length < 3) continue;
...
async function processStorms(wrecks: any[]) {
  try {
...
async function processOilSpillDetection(wrecks: any[]) {
  try {
